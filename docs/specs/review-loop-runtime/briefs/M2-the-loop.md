...
holds **43** crafted cases (`::test_j10_case_inventory_matches_this_brief` fails when
...
seven wrapper forms (`timeout`, and the six option-bearing ones round 4 measured); eight of
line structure (continuation, continued comment, trailing comment, a `#` inside quotes, a
here-string, and three nested markers — alphabetic, digit and non-alphabetic info strings,
none of which closes a fence); four deliberate silences — `eval` and `bash -c`, which
