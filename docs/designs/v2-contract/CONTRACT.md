...
- Non-convergence is currently unhandled by design: nothing stops an execution session that thrashes or starts bending code toward an unreachable test. Fixed budgets were rejected (any static limit halts legitimate long runs far more often than it catches pathology). Five executor runs in the pilot — four delivered, one stopped at the escape valve — and none bent code toward an unreachable test; the question stays open, and detection, if ever needed, should key on symptoms, not duration.
