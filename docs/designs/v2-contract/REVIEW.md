...
| 09-01, 09-08, 09-11 | 1Password approval is silent and per-process; ten `op read` calls, each a prompt, none announced. | Milestone: announce *before* resolving ("resolving N secrets via 1Password — approve the prompt"); resolve all refs in **one** `op` process (`op inject` over a generated template) so it is one approval. |
