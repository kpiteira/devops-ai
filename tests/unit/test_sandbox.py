...
class TestGenerateOverride:
    def test_code_mounts(self, tmp_path: Path) -> None:
        ...
        result = generate_override(config, slot, wt, main_repo, slot_dir)
        content = result.read_text()
    ...
    def test_override_includes_observability_network(
        self, tmp_path: Path
    ) -> None:
        """Override always includes devops-ai-observability network."""
        config = _config(
            code_mounts=[MountEntry("src/", "/app/src")],
            code_mount_targets=["app"],
        )
    ...
    def test_override_otel_endpoint(self, tmp_path: Path) -> None:
        """OTEL endpoint points to devops-ai-jaeger:4317."""
        config = _config(
            code_mounts=[MountEntry("src/", "/app/src")],
            code_mount_targets=["app"],
        )
    ...
    def test_override_otel_namespace(self, tmp_path: Path) -> None:
        """OTEL namespace matches <project>-slot-<N> pattern."""
        config = _config(
            code_mounts=[MountEntry("src/", "/app/src")],
            code_mount_targets=["app"],
        )
        slot = _slot(slot_id=3)
        wt = tmp_path / "worktree"
        wt.mkdir()
        main_repo = tmp_path / "main"
        main_repo.mkdir()
        ...
        result = generate_override(config, slot, wt, main_repo, slot_dir)
        content = result.read_text()
        assert "service.namespace=myproj-slot-3" in content

    def test_override_custom_otel_vars(self, tmp_path: Path) -> None:
        """Config with [sandbox.otel] uses custom var names."""
        config = _config(
            code_mounts=[MountEntry("src/", "/app/src")],
            code_mount_targets=["app"],
        )
        config.otel_endpoint_var = "MY_OTEL_ENDPOINT"
        config.otel_namespace_var = "MY_OTEL_NS"
        slot = _slot()
        wt = tmp_path / "worktree"
        wt.mkdir()
        main_repo = tmp_path / "main"
        ...
        result = generate_override(config, slot, wt, main_repo, slot_dir)
        content = result.read_text()
        assert "MY_OTEL_ENDPOINT=http://devops-ai-jaeger:4317" in content
        assert "MY_OTEL_NS=service.namespace=myproj-slot-1" in content

    def test_override_services_join_observability(
        self, tmp_path: Path
    ) -> None:
        """Target services have both default and devops-ai-observability."""
        config = _config(
            code_mounts=[MountEntry("src/", "/app/src")],
            code_mount_targets=["app"],
        )
