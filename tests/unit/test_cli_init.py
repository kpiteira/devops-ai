...
class TestInitQualityArtifacts:
    ...
    def test_updates_kinfra_generated_artifacts(self, tmp_path: Path) -> None:
        """Overwrites existing artifacts that have the kinfra marker."""
        from devops_ai.cli.init_cmd import init_command

        self._setup_project(tmp_path)
        # Pre-create with the kinfra marker (simulates old generated file)
        ...
        # Should be updated, not the old content
        assert "old" not in content
        assert "make check" in content or "uv run" in content
