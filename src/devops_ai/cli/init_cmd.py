...
_GENERATED_MARKER = "generated by kinfra init"


def _is_kinfra_generated(path: Path) -> bool:
    """Check if a file was generated by kinfra init (has marker comment)."""
    try:
        first_line = path.read_text().split("\n", 1)[0]
        return _GENERATED_MARKER in first_line.lower()
    ...
    for path, content in artifacts:
        if path.exists():
            if _is_kinfra_generated(path):
                ...
                typer.echo(
                    f"  updated: {path.relative_to(project_root)}"
                )
            else:
                typer.echo(f"  skipped: {path.name} (user-managed)")
            continue
