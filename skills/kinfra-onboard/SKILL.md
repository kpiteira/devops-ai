...
6. **Check quality infrastructure:**
   - Check for `Makefile`, `Justfile`, `.githooks/pre-commit`, `.github/workflows/ci.yml`
   - Report missing quality artifacts (generated by `kinfra init`)
...
4. **Quality infrastructure works** (if Makefile exists):
   ```bash
   make check
   ```
