...
- `kobserve` observer seat: independent verification and the *For the human* gate before a milestone merges
...
- [ ] **Required checks on main** (#28) — main has no branch protection today: enable the ruleset with `check` (and `integration`, once #36 lands) as required status checks; `kinfra init` prints the one-time instruction and generates a CODEOWNERS entry for the guard and the workflow. Decided 2026-09-13
- [ ] **Integration tests in CI** (#36) — a separate `integration` job generated by `kinfra init` for projects with `tests/integration/`; in progress
...
- [ ] **Review loop, second pilot** — the redesigned loop has run only on skill-text, lockfile and test PRs; the first real milestone PR under it is the measurement (EVOLUTIONS.md #3)
